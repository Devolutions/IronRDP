...
use crate::rdp::server_license::{
    BasicSecurityHeader, BasicSecurityHeaderFlags, LicenseHeader, PreambleFlags, PreambleType,
...
lazy_static! {
    ...
    pub static ref CLIENT_HARDWARE_IDENTIFICATION: ClientHardwareIdentification =
        ClientHardwareIdentification {
    ...
    pub static ref CLIENT_PLATFORM_CHALLENGE_RESPONSE: ClientPlatformChallengeResponse =
        ClientPlatformChallengeResponse {
            license_header: LicenseHeader {
                ...
                preamble_message_size: (CLIENT_PLATFORM_CHALLENGE_RESPONSE_BUFFER.len()
                    - BASIC_SECURITY_HEADER_SIZE) as u16,
            },
...
fn from_buffer_correctly_parses_platform_challenge_response_data() {
    assert_eq!(
        *RESPONSE,
        PlatformChallengeResponseData::from_buffer(
...
fn from_buffer_correctly_parses_client_hardware_identification() {
    assert_eq!(
        *CLIENT_HARDWARE_IDENTIFICATION,
        ClientHardwareIdentification::from_buffer(CLIENT_HARDWARE_IDENTIFICATION_BUFFER.as_ref())
            .unwrap()
    );
}
...
fn from_buffer_correctly_parses_client_platform_challenge_response() {
    assert_eq!(
        *CLIENT_PLATFORM_CHALLENGE_RESPONSE,
        ClientPlatformChallengeResponse::from_buffer(
...
fn challenge_response_creates_from_server_challenge_and_encryption_data_correctly() {
    ...
    let server_challenge = ServerPlatformChallenge {
        license_header: LicenseHeader {
            ...
            preamble_flags: PreambleFlags::empty(),
            preamble_version: PreambleVersion::V3,
            preamble_message_size: (encrypted_platform_challenge.len()
                + mac_data.len()
