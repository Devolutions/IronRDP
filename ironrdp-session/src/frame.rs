    ...
    RdpError: From<<T as PduParsing>::Error>,
{
    fn encode(self, mut stream: impl io::Write) -> Result<(), RdpError> {
        ironrdp_core::DataHeader::new(self.0.buffer_length())
    ...
    fn decode(mut stream: impl io::Read) -> Result<Self, RdpError> {
        let data_header =
            ironrdp_core::DataHeader::from_buffer(&mut stream).map_err(ironrdp_core::RdpError::X224Error)?;
...
#[derive(Clone, Debug)]
pub struct McsFrame {
    pub pdu: ironrdp_core::McsPdu,
    pub data: Bytes,
}

impl Frame for McsFrame {
    fn encode(self, mut stream: impl io::Write) -> Result<(), RdpError> {
        let data_header = ironrdp_core::DataHeader::new(self.pdu.buffer_length() + self.data.len());
    ...
    fn decode(mut stream: impl io::Read) -> Result<Self, RdpError> {
        let data_header = ironrdp_core::DataHeader::from_buffer(&mut stream)?;

        let mcs_pdu = ironrdp_core::McsPdu::from_buffer(&mut stream)?;
...
impl Frame for SendDataInfoFrame {
    fn encode(self, stream: impl io::Write) -> Result<(), RdpError> {
        ...
        McsFrame {
            pdu: ironrdp_core::McsPdu::SendDataRequest(send_data_context),
            data: self.data,
        }
        .encode(stream)
    }
    ...
    fn decode(stream: impl io::Read) -> Result<Self, RdpError> {
        ...
        match mcs_frame.pdu {
            ironrdp_core::McsPdu::SendDataIndication(send_data_context) => Ok(Self {
                channel_ids: ChannelIdentificators {
...
pub struct ShareControlFrame {
    ...
    pub pdu: ironrdp_core::ShareControlPdu,
    pub data: Bytes,
}
...
pub struct ShareDataFrame {
    ...
    pub share_id: u32,
    pub pdu_source: u16, // NOTE: looks like this is always equal to channel_ids.initiatior_id
    pub pdu: ironrdp_core::ShareDataPdu,
}
...
impl Frame for ShareDataFrame {
    fn encode(self, stream: impl io::Write) -> Result<(), RdpError> {
        ...
        ShareControlFrame {
            ...
            share_id: self.share_id,
            pdu_source: self.pdu_source,
            pdu: ironrdp_core::ShareControlPdu::Data(share_data_header),
            data: Bytes::new(),
        }
        .encode(stream)
    }
    ...
    fn decode(stream: impl io::Read) -> Result<Self, RdpError> {
        ...
        if let ironrdp_core::ShareControlPdu::Data(share_data_header) = frame.pdu {
            Ok(Self {
    ...
    RdpError: From<<T as PduParsing>::Error>,
{
    ...
    fn decode(stream: impl io::Read) -> Result<Self, RdpError> {
        ...
        let channel_ids = match mcs_frame.pdu {
            ironrdp_core::McsPdu::SendDataIndication(send_data_context) => ChannelIdentificators {
                initiator_id: send_data_context.initiator_id,
                channel_id: send_data_context.channel_id,
            },
