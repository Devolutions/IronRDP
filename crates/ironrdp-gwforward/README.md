...
- **Local port forward** ([`run_port_forward`]) — SSH `-L`-style.
  Listen on a local port and relay each inbound connection to a fixed `host:port` through the gateway.
...
Callers supply a [`GatewayTunnelConfig`] (gateway endpoint, credentials, and client name) plus a local listen address.
The crate owns the accept loops and the per-connection tunnel setup; it does not manage RDP sessions.

This crate uses the modern WebSocket MS-TSGU path exposed by `ironrdp-mstsgu`.
