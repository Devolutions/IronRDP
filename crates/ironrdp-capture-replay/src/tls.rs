...
fn tls_prf_sha256(secret: &[u8], label: &[u8], seed: &[u8], length: usize) -> Result<Vec<u8>, ReplayError> {
    ...
    let mut a = <Hmac<Sha256> as KeyInit>::new_from_slice(secret)
        .map_err(|_| ReplayError::UnsupportedTls)?
    ...
    while output.len() < length {
        let mut hmac = <Hmac<Sha256> as KeyInit>::new_from_slice(secret).map_err(|_| ReplayError::UnsupportedTls)?;
        hmac.update(&a);
        hmac.update(&label_seed);
        output.extend_from_slice(&hmac.finalize().into_bytes());
        a = <Hmac<Sha256> as KeyInit>::new_from_slice(secret)
            .map_err(|_| ReplayError::UnsupportedTls)?
            .chain_update(&a)
            .finalize()
            .into_bytes()
...
fn tls_prf_sha384(secret: &[u8], label: &[u8], seed: &[u8], length: usize) -> Result<Vec<u8>, ReplayError> {
    ...
    label_seed.extend_from_slice(label);
    label_seed.extend_from_slice(seed);
    let mut a = <Hmac<Sha384> as KeyInit>::new_from_slice(secret)
        .map_err(|_| ReplayError::UnsupportedTls)?
        .chain_update(&label_seed)
        .finalize()
        .into_bytes()
    ...
    let mut output = Vec::with_capacity(length);
    while output.len() < length {
        let mut hmac = <Hmac<Sha384> as KeyInit>::new_from_slice(secret).map_err(|_| ReplayError::UnsupportedTls)?;
        hmac.update(&a);
        hmac.update(&label_seed);
        output.extend_from_slice(&hmac.finalize().into_bytes());
        a = <Hmac<Sha384> as KeyInit>::new_from_slice(secret)
            .map_err(|_| ReplayError::UnsupportedTls)?
            .chain_update(&a)
            .finalize()
            .into_bytes()
