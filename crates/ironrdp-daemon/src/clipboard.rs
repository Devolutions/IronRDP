...
//! (`CF_UNICODETEXT`), images (`CF_DIB`/`CF_DIBV5`, stored as PNG), HTML fragments (the
//! registered `HTML Format`), and files (the `FileGroupDescriptorW` file-list mechanism). A
//! headless daemon has no host clipboard of its own, so this backend holds the last content
...
//! daemon's IPC handlers. Content is a single logical item at a time (text, image, HTML, or
//! files, not several at once), matching how each `clipboard-set*` call replaces whatever was
//! there before.
//!
...
impl CliprdrBackend for AgentCliprdrBackend {
    ...
    fn on_remote_copy(&mut self, available_formats: &[ClipboardFormat]) {
        ...
        // Prefer the richest representation offered: files over image over HTML over text, DIBV5
        // over DIB.
