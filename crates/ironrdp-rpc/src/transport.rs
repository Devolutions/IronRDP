...
//!
//! RPC peers talk over a platform-native local transport:
//!
//! - **Unix**: a [`tokio::net::UnixListener`]/[`tokio::net::UnixStream`] at a named
//!   `$XDG_RUNTIME_DIR/<name>-<uid>.sock`, falling back to `/tmp` when `XDG_RUNTIME_DIR` is unset.
//! - **Windows**: a named pipe at `\\.\pipe\<name>-<user>`, restricted to the current user.
//!
    ...
    M: DecodeOwned,
{
    ...
    let len = usize::try_from(u32::from_le_bytes(len_buf)).context("frame length does not fit this platform")?;
...
pub async fn open_stream(endpoint: &Endpoint, request: &Request) -> anyhow::Result<ClientStream> {
    let mut stream = connect(endpoint)
        .await
        .with_context(|| format!("connect to RPC server at {endpoint}"))?;
...
mod imp {
    ...
    /// Returns a named per-user endpoint.
    ...
    pub async fn prepare_endpoint(endpoint: &Endpoint) -> anyhow::Result<()> {
        ...
        // No server answered, so the path may be a stale socket. Guard against deleting an
        // unrelated regular file (or following a symlink) when an explicit endpoint is used.
        use std::os::unix::fs::FileTypeExt as _;
        let metadata =
            std::fs::symlink_metadata(&endpoint.0).with_context(|| format!("stat IPC endpoint {endpoint}"))?;
        if !metadata.file_type().is_socket() {
            bail!("refusing to remove {endpoint}: path exists and is not a socket");
        }
...
mod imp {
    ...
    pub async fn prepare_endpoint(_endpoint: &Endpoint) -> anyhow::Result<()> {
        Ok(())
    }
