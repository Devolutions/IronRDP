...
impl RdpsndServer {
    ...
    pub fn wave(&mut self, data: Vec<u8>, ts: u32) -> PduResult<RdpsndSvcMessages> {
        ...
        // the server doesn't wait for wave confirm, apparenyly FreeRDP neither
        let pdu = if version >= pdu::Version::V8 {
            pdu::Wave2Pdu {
        ...
        Ok(RdpsndSvcMessages::new(vec![
            pdu::ServerAudioOutputPdu::Wave2(pdu).into()
        ]))
    }
