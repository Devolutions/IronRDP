...
///
/// The same type can implement both `MakeDynamicVirtualChannel` and `DynamicVirtualChannel` traits when appropriate
/// (unit structs should generally do this).
pub trait MakeDynamicVirtualChannel: fmt::Debug + Send + Sync {
    fn channel_name(&self) -> &str;

    fn make_dynamic_channel(&self, channel_id: u32) -> Box<dyn DynamicVirtualChannel>;
}

assert_obj_safe!(MakeDynamicVirtualChannel);
...
impl WithDrdynvc {
    ...
    pub fn with_dynamic_channel<T>(mut self, with_channel: T) -> Self
    where
        T: MakeDynamicVirtualChannel + 'static,
    {
        let channel_name = with_channel.channel_name().to_owned();
        self.dynamic_channels.insert(channel_name, Arc::new(with_channel));
        self
    }
}
