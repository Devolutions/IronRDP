...
impl DeviceInfo {
    pub fn new(
        ...
        active_config: Option<UsbConfigInfo>,
...
impl UsbDeviceLocation {
    pub fn new(bus_number: u8, address: u8, port_numbers: Vec<u8>) -> Self {
        Self {
            bus_number,
            address,
            port_numbers,
...
impl UsbDeviceDescriptorInfo {
    pub const fn new(
        vendor_id: u16,
        product_id: u16,
        device_version: UsbBcdVersion,
        usb_version: UsbBcdVersion,
...
impl UsbConfigInfo {
    pub fn new(interfaces: Vec<UsbInterfaceInfo>) -> Self {
        Self { interfaces }
    }
}
...
impl UsbInterfaceInfo {
    pub const fn new(class_codes: UsbClassCodes) -> Self {
        Self { class_codes }
    }
}
...
impl UsbClassCodes {
    pub const PER_INTERFACE: Self = Self::new(0x00, 0x00, 0x00);

    pub const fn new(class_code: u8, sub_class_code: u8, protocol_code: u8) -> Self {
        Self {
            class_code,
            sub_class_code,
            protocol_code,
