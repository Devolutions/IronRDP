...
impl DeferredOperations {
    ...
    pub(super) fn cancel_file(&mut self, device_id: u32, file_id: u32, handle: &FileHandle) -> Vec<SvcMessage> {
        ...
        for operation_id in &operation_ids {
            ...
            let acquired = operation.cancellation.acquired.load(Ordering::Acquire);
            let request = operation.request;
            join_worker(operation.worker, file_id, "lock");
            if acquired && let Err(error) = handle.unlock_ranges(&operation.ranges) {
    ...
    loop {
        ...
        match handle.lock_ranges(&ranges, exclusive) {
            Ok(()) => {
                ...
                if cancellation.cancelled.load(Ordering::Acquire) {
                    if let Err(error) = handle.unlock_ranges(&ranges) {
                        tracing::warn!(error = %error, file_id = request.file_id, "Failed to release cancelled deferred RDPDR lock");
                    }
                    return;
                }
...
fn cancel_directory_notification(notification: PendingDirectoryNotification) {
    ...
    while !notification.cancellation.finished.load(Ordering::Acquire) {
        ...
        let _ = unsafe {
            CancelIoEx(
                windows::Win32::Foundation::HANDLE(notification.handle as *mut core::ffi::c_void),
                None,
            )
        };
...
mod tests {
    ...
    fn cancelling_a_waiting_lock_completes_once_and_discards_a_late_worker_result() {
        ...
        let cancelled = operations.cancel_file(1, 7, &locking_handle);
    ...
    fn closing_a_directory_watch_returns_a_successful_empty_completion() {
...

        {
            ...
            let cancelled = operations.cancel_file(1, 7, &directory_handle);
            assert_eq!(cancelled.len(), 1);
            let response = cancelled
                .into_iter()
                .next()
