...
pub(crate) fn close(backend: &mut WindowsRdpdrBackend, req: DeviceCloseRequest) -> PduResult<Vec<SvcMessage>> {
    ...
    let status = if matches!(
        ...
        messages.extend(backend.cancel_deferred_file_operations(
...
fn query_information_inner(
    ...
    match req.file_info_class_lvl {
        ...
        FileInformationClassLevel::FILE_STREAM_INFORMATION => file
            .handle
            .query_stream_information()
            .map(FileStreamInformation::from_buffer)
            .map(Into::into)
            .map_err(from_ntstatus),
