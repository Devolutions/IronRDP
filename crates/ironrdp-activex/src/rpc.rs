        ...
        let join = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                match runtime {
                    Ok(runtime) => {
                        if let Err(error) =
                            runtime.block_on(serve(endpoint, shared, dispatcher, shutdown_rx, shutdown_for_listener))
                        {
        ...
        tokio::select! {
            result = listener.accept() => {
                ...
                tokio::spawn(async move {
                    if let Err(error) = handle_connection(stream, shared, dispatcher, shutdown).await {
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let listener_shutdown = matches!(request, Request::Shutdown);
    ...
    if listener_shutdown {
        // The caller has received its success reply. This only stops the RPC listener; it never
        // touches the STA control or its RDP session.
        let _ = shutdown.send(());
    }
    Ok(())
}
