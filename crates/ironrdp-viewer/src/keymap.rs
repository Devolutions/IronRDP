...
mod tests {
    ...
    #[test]
    fn maps_physical_keys_to_set_1_scancodes() {
        assert_eq!(
            map_key_code(KeyCode::KeyA).map(|scancode| scancode.as_u8()),
            Some((false, 0x1E))
        );
        assert_eq!(
            map_key_code(KeyCode::Digit1).map(|scancode| scancode.as_u8()),
            Some((false, 0x02))
        );
        assert_eq!(
            map_key_code(KeyCode::Slash).map(|scancode| scancode.as_u8()),
            Some((false, 0x35))
        );
        assert_eq!(
            map_key_code(KeyCode::IntlBackslash).map(|scancode| scancode.as_u8()),
            Some((false, 0x56))
        );
        assert_eq!(
            map_key_code(KeyCode::F12).map(|scancode| scancode.as_u8()),
            Some((false, 0x58))
        );
        assert_eq!(
            map_key_code(KeyCode::Enter).map(|scancode| scancode.as_u8()),
            Some((false, 0x1C))
        );
        assert_eq!(
            map_key_code(KeyCode::ArrowLeft).map(|scancode| scancode.as_u8()),
            Some((true, 0x4B))
        );
        assert_eq!(
            map_key_code(KeyCode::Numpad4).map(|scancode| scancode.as_u8()),
            Some((false, 0x4B))
        );
        assert_eq!(
            map_key_code(KeyCode::NumpadEnter).map(|scancode| scancode.as_u8()),
            Some((true, 0x1C))
        );
    }

    #[test]
    fn preserves_keyboard_input_lifecycle() {
        let key_a = map_key_code(KeyCode::KeyA).expect("standard physical key");
        let arrow_left = map_key_code(KeyCode::ArrowLeft).expect("extended physical key");
        let mut database = Database::new();

        let events = database.apply([
            Operation::KeyPressed(key_a),
            Operation::KeyPressed(arrow_left),
            Operation::KeyPressed(arrow_left),
            Operation::KeyReleased(key_a),
            Operation::KeyReleased(arrow_left),
        ]);
        ...
        assert_eq!(
            ...
            [
                ...
                FastPathInputEvent::KeyboardEvent(KeyboardFlags::EXTENDED, 0x4B),
                FastPathInputEvent::KeyboardEvent(KeyboardFlags::EXTENDED | KeyboardFlags::RELEASE, 0x4B),
                FastPathInputEvent::KeyboardEvent(KeyboardFlags::EXTENDED, 0x4B),
    ...
    #[test]
    fn leaves_modifiers_and_unrepresentable_keys_to_explicit_handling() {
        assert!(is_modifier(KeyCode::ControlRight));
        assert!(is_modifier(KeyCode::SuperLeft));
