...
mod crash_repro {
    ...
    #[test]
    fn test_bulk_round_trip_crash_repro() {
        let crash_data: Vec<u8> = vec![
            255, 248, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
            40, 42, 42, 42, 42, 42, 42, 42, 42,
        ...
        let (algo_byte, src) = crash_data.split_first().unwrap();
        ...
        let mut sender = BulkCompressor::new(algo).unwrap();
        let (compressed_size, compress_flags) = sender.compress(src).unwrap();
        
        eprintln!("compressed_size={}, compress_flags=0x{:X}", compressed_size, compress_flags);
        
        let payload = if compress_flags & flags::PACKET_COMPRESSED == 0 {
            src.to_vec()
        } else {
            sender.compressed_data(compressed_size).to_vec()
        };
        
        eprintln!("payload len={}", payload.len());
        if payload.len() >= 2 {
            eprintln!("payload[0]=0x{:X}, payload[1]=0x{:X}", payload[0], payload[1]);
        }
        
        let mut receiver = BulkCompressor::new(algo).unwrap();
        match receiver.decompress(&payload, compress_flags) {
            Ok(decompressed) => {
                eprintln!("decompressed len={}", decompressed.len());
                assert_eq!(decompressed, src, "round-trip byte-equality failed");
            }
            Err(e) => {
