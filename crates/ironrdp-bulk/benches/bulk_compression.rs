...
fn bench_compress_decompress(c: &mut Criterion, ct: CompressionType, data: &[u8], label: &str) {
    ...
    let mut test_comp = BulkCompressor::new(ct).unwrap();
    let (test_size, test_flags) = test_comp.compress(data).unwrap();
    let compresses = test_flags & flags::PACKET_COMPRESSED != 0;

    if compresses {
        let compressed = test_comp.compressed_data(test_size).to_vec();
        let ratio = data.len() as f64 / compressed.len() as f64;
        eprintln!(
            "{name}/{label}: {len} -> {clen} bytes ({ratio:.2}x)",
            len = data.len(),
            clen = compressed.len(),
        );
        ...
        eprintln!("{name}/{label}: data not compressible, skipping");
    }
}
