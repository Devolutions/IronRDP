...
mod windows_main {
    ...
    use ironrdp_server::tokio_rustls::{TlsAcceptor, rustls};
    use ironrdp_server::{
    ...
    use ironrdp_wtsprotocol_ipc::{
        DEFAULT_MAX_FRAME_SIZE, ProviderCommand, ServiceEvent, default_pipe_name, pipe_path, resolve_pipe_name_from_env,
    };
    ...
    use windows::Win32::Foundation::{
        ERROR_NOT_ALL_ASSIGNED, GetLastError, HANDLE, HLOCAL, LUID, LocalFree, SetLastError, WAIT_OBJECT_0,
    ...
    use windows::Win32::System::Threading::{
        CREATE_NO_WINDOW, CreateEventW, CreateProcessAsUserW, EVENT_MODIFY_STATE, GetCurrentProcess, OpenEventW,
        OpenProcess, OpenProcessToken, PROCESS_INFORMATION, PROCESS_QUERY_LIMITED_INFORMATION, STARTUPINFOW, SetEvent,
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        GetDesktopWindow, GetSystemMetrics, SM_CXSCREEN, SM_CYSCREEN, SetCursorPos,
    };
    use windows::core::{PCWSTR, PWSTR, w};

    use windows::Win32::Security::{
        AdjustTokenPrivileges, DuplicateTokenEx, LUID_AND_ATTRIBUTES, LookupPrivilegeValueW, SE_PRIVILEGE_ENABLED,
        SecurityImpersonation, SetTokenInformation, TOKEN_ADJUST_PRIVILEGES, TOKEN_ADJUST_SESSIONID,
        TOKEN_ASSIGN_PRIMARY, TOKEN_DUPLICATE, TOKEN_PRIVILEGES, TOKEN_QUERY, TokenPrimary, TokenSessionId,
    };
    ...
    impl RdpServerDisplayUpdates for GdiDisplayUpdates {
        async fn next_update(&mut self) -> anyhow::Result<Option<DisplayUpdate>> {
            ...
            match captured {
                CapturedFrame::PreEncoded(surface) => Ok(Some(DisplayUpdate::PreEncodedSurface(surface))),
                CapturedFrame::Raw(bitmap) => {
    ...
    async fn run_server_once(
        ...
        match handle_client(&mut server, state).await {
            Ok(commands_processed) if commands_processed == 0 => {
                *empty_count += 1;
                if *empty_count == 1 || (*empty_count <= 10 && *empty_count % 5 == 0) || *empty_count % 100_000 == 0 {
                    info!(
                        empty_disconnects = *empty_count,
                        "Client disconnected without sending commands"
                    );
                }
            }
            Ok(_) => {
    ...
    async fn handle_client(pipe: &mut named_pipe::NamedPipeServer, state: &mut ServiceState) -> anyhow::Result<u32> {
