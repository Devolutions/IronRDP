...
///
/// `recipe` is the Windows Sandbox configuration XML. When it is absent, the server uses its
/// default configuration. `sandbox_id` lets callers select a stable ID; otherwise the server
/// assigns one.
