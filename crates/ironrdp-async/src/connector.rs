use ironrdp_connector::credssp::{CredsspProcessGenerator, CredsspSequenceTrait, KerberosConfig};
...
use ironrdp_connector::{
    ...
    ConnectorResult, SecurityConnector, Sequence, ServerName, State as _,
};
    ...
    S: Sync + FramedRead + FramedWrite,
{
    ...
    while !connector.should_perform_security_upgrade() {
        single_sequence_step(framed, connector.as_sequence(), &mut buf).await?;
    }
...
pub async fn perfrom_credssp<S>(
    _: Upgraded,
    connector: &mut dyn ConnectorCore,
    framed: &mut Framed<S>,
