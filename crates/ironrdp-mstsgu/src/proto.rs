...
impl<'a> Decode<'a> for PktHdr {
    fn decode(src: &mut ReadCursor<'a>) -> ironrdp_core::DecodeResult<Self> {
        ...
        let mty = PktTy::try_from(ty)
            .map_err(|_| unsupported_value_err("PktHdr::ty", "ty", format!("0x{ty:x}"), Some(src.pos())))?;
        ...
        Ok(HandshakeRespPkt {
            ...
            _extended_auth: {
                ...
                HttpExtendedAuth::from_bits(raw).ok_or_else(|| {
                    unsupported_value_err("HandshakeResp", "extended_auth", format!("0x{raw:x}"), Some(src.pos()))
                })?
            },
        })
    }
