        ...
        match self {
            PacketIo::WebSocket { stream, .. } => {
                let msg = match stream.next().await {
                    // A clean close or an exhausted stream ends the read with an EOF-class
                    // error so callers observe end-of-stream rather than a hard failure.
                    None => {
                        return Err(Error::new("gateway tunnel closed", GwErrorKind::PacketEof));
                    }
                    Some(Ok(msg)) => msg,
