...
impl RdpServerBuilder<BuilderDone> {
    ...
    ///
    /// See [`RdpServerOptions::preempt_existing_session`] for the exact
    /// mechanics, including what "authenticated" means per security mode and
    /// why an unauthenticated peer can never evict a live session.
