...
impl Framebuffer {
    pub fn new(width: NonZeroU16, height: NonZeroU16, format: PixelFormat) -> Self {
        let stride = format.bytes_per_pixel() as usize * width.get() as usize;
        let data = vec![0; stride * width.get() as usize];
    ...
    pub fn update(&mut self, bitmap: &BitmapUpdate) {
        ...
        let mut data: BytesMut = self.data.take().unwrap().into();
