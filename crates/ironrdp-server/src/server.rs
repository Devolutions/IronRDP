...
impl RdpServer {
    ...
    async fn client_loop<S>(&mut self, mut framed: Framed<S>, mut result: AcceptorResult) -> Result<()>
    where
        S: FramedWrite + FramedRead,
    {
        ...
        self.static_channels = std::mem::take(&mut result.static_channels);
        for (_type_id, channel, channel_id) in self.static_channels.iter_mut() {
            let Some(channel_id) = channel_id else {
                continue;
            };
            let response_svcs = channel.start()?;
            let response = server_encode_svc_messages(response_svcs, channel_id, result.user_channel_id)?;
