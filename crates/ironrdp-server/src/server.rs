...
async fn negotiate_candidate(
    ...
    match negotiate_and_authenticate(&ctx.opts.security, &mut acceptor, stream, TransportTls::Managed).await {
        Ok(Some(transport)) => {
            ...
            Some((Box::new(NegotiatedCandidate { transport, acceptor }), peer))
        }
        Ok(None) => {
            debug!(
                ?peer,
...
impl RdpServer {
    ...
    /// Drop any `EvictedByOtherConnection` still queued on the server-global
    /// event channel, putting every other event back in arrival order.
    ///
