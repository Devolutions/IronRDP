...
#[test]
fn initiate_copy_preserves_file_list_during_upload() {
    ...
    *cliprdr.__test_local_file_list_format_id_mut() = Some(file_format_id);
    ...
    // Bookkeeping: file list preserved during active upload
    assert!(cliprdr.__test_local_file_list().is_some());
    assert_eq!(cliprdr.__test_local_file_list_format_id(), Some(file_format_id));
}
...
fn stale_request_cleanup_after_timeout() {
    ...
    *cliprdr.__test_state_mut() = CliprdrState::Ready;
    ...
    let _messages = cliprdr.cleanup_expired_locks().unwrap();
...
#[test]
fn stale_request_cleanup_via_automatic_sweep() {
    let responses = Arc::new(Mutex::new(Vec::new()));
    let unlocks = Arc::new(Mutex::new(Vec::new()));
    let backend = TimedRecordingBackend::new(Arc::clone(&responses), Arc::clone(&unlocks));
    ...
    let mut cliprdr: CliprdrClient = Cliprdr::with_all_config(
        ...
        core::time::Duration::from_secs(60),
        core::time::Duration::from_secs(3600),
        Some(core::time::Duration::from_millis(50)), // 50ms check interval
    ...
    *cliprdr.__test_state_mut() = CliprdrState::Ready;
    ...
    // Advance past both the check interval and transfer timeout
    cliprdr
        .downcast_backend::<TimedRecordingBackend>()
        .unwrap()
        .advance_ms(300);

    let messages = cliprdr.maybe_cleanup_locks();
    assert!(messages.is_empty());
    assert_eq!(cliprdr.__test_sent_file_contents_requests().len(), 0);
