...
/// [MS-RDPECLIP] 3.1.5.4.5 - a request carrying a `clipDataId` is validated
/// against the list that lock covers.
///
/// A Lock PDU asks the remote to keep its File Stream data alive across a
/// clipboard change, and 3.1.5.4.5 requires that data to service a request
/// carrying the id. Validating such a request against the *current* remote file
/// list defeats that: replacing a two-file selection with a one-file one made an
