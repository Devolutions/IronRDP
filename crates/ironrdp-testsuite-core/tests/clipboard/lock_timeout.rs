//! Tests for lock timeout, cleanup throttling, and automatic/manual
//! cleanup behavior.
//!
...
fn lock_activity_prevents_timeout() {
    ...
    *cliprdr.__test_remote_file_list_mut() = Some(sized_file_list(&[("test.txt", 1000)]));
    ...
    let messages: Vec<SvcMessage> = Vec::from(cliprdr.cleanup_expired_locks().unwrap());
...
// -- Automatic cleanup throttling ------------------------------------

#[test]
fn automatic_cleanup_throttling() {
    use core::time::Duration;

    // Use inactivity timeout (10ms) shorter than throttle interval (500ms)
    // so we can test "lock eligible but throttled" scenarios.
    let mut cliprdr: CliprdrClient = Cliprdr::with_cleanup_config(
        Box::new(LockingBackend::new()),
        Duration::from_millis(10),        // Inactivity timeout: 10ms
        Duration::from_secs(60),          // Max lifetime: 60s
        Some(Duration::from_millis(500)), // Throttle interval: 500ms
    );

    assert_eq!(
        cliprdr.__test_cleanup_check_interval(),
    ...
    *cliprdr.__test_capabilities_mut() = Capabilities::new(
    ...
    // Create lock, expire it, advance past both inactivity (10ms) and
    // throttle (500ms). The process() calls during lock creation and expiry set
    // last_cleanup_check, so we must advance past the throttle for the first
    // explicit cleanup to run.
    let id1 = process_file_format_list(&mut cliprdr);
    expire_via_text_format_list(&mut cliprdr);
    cliprdr.downcast_backend::<LockingBackend>().unwrap().advance_ms(600);

    // First cleanup via initiate_copy should work
    let _messages = cliprdr.initiate_copy(&formats).unwrap();
    assert!(
        cliprdr.__test_last_cleanup_check().is_some(),
        "Cleanup timestamp should be set"
    );
    assert!(
        !cliprdr.__test_outgoing_locks().contains_key(&id1),
        "Lock should have been cleaned up"
    );

    // Create another lock, expire it, advance past inactivity but NOT throttle
    let id2 = process_file_format_list(&mut cliprdr);
    expire_via_text_format_list(&mut cliprdr);
    cliprdr.downcast_backend::<LockingBackend>().unwrap().advance_ms(20);

    // Should be throttled (only ~20ms since last cleanup < 500ms throttle)
    let _messages = cliprdr.initiate_copy(&formats).unwrap();
    assert!(
        cliprdr.__test_outgoing_locks().contains_key(&id2),
        "Lock should NOT be cleaned up due to throttling"
    );

    // Advance past throttle interval (500ms)
    cliprdr.downcast_backend::<LockingBackend>().unwrap().advance_ms(600);

    let _messages = cliprdr.initiate_copy(&formats).unwrap();
    assert!(
        !cliprdr.__test_outgoing_locks().contains_key(&id2),
        "Cleanup after throttle interval should remove lock"
    );
}

#[test]
fn automatic_cleanup_api_integration() {
    use core::time::Duration;

    let mut cliprdr: CliprdrClient = Cliprdr::with_cleanup_config(
        Box::new(LockingBackend::new()),
        Duration::from_millis(1), // Very short inactivity timeout
        Duration::from_secs(60),
        Some(Duration::from_millis(1)), // Very short throttle for testing
    );
    ...
    *cliprdr.__test_capabilities_mut() = Capabilities::new(
    ...
    // Advance clock past the 1ms inactivity timeout
    cliprdr.downcast_backend::<LockingBackend>().unwrap().advance_ms(10);

    let initial_count = cliprdr.__test_outgoing_locks().len();
    assert_eq!(initial_count, 1, "Should have 1 lock initially");

    // Trigger automatic cleanup via initiate_copy API
    let formats = vec![ClipboardFormat::new(ClipboardFormatId::new(13))];
    let _messages = cliprdr.initiate_copy(&formats).unwrap();

    assert!(
        cliprdr.__test_last_cleanup_check().is_some(),
        "Cleanup should have been invoked during API call"
    );
}

#[test]
fn automatic_cleanup_disabled() {
    use core::time::Duration;
    ...
    let mut cliprdr: CliprdrClient = Cliprdr::with_cleanup_config(
        ...
        Duration::from_secs(60),
        None, // Disabled automatic cleanup
    );
    ...
    *cliprdr.__test_capabilities_mut() = Capabilities::new(
    ...
    // Expire via text FormatList and advance clock
    expire_via_text_format_list(&mut cliprdr);
    cliprdr.downcast_backend::<LockingBackend>().unwrap().advance_ms(10);
    ...
    assert_eq!(
        cliprdr.__test_cleanup_check_interval(),
        None,
        "Automatic cleanup should be disabled"
    );

    // initiate_copy's maybe_cleanup_locks should do nothing since disabled
    let formats = vec![ClipboardFormat::new(ClipboardFormatId::new(13))];
    let _messages = cliprdr.initiate_copy(&formats).unwrap();
    assert_eq!(
        cliprdr.__test_outgoing_locks().len(),
        1,
        "Lock should not be cleaned up"
    );

    // Behavior: manual cleanup still works and returns an Unlock PDU
    let messages: Vec<SvcMessage> = cliprdr.cleanup_expired_locks().unwrap().into();
    assert_eq!(
        messages.len(),
        1,
        "manual cleanup should work even when auto-cleanup disabled"
    );
    decode_pdu!(messages[0] => _bytes, pdu);
    match pdu {
        ClipboardPdu::UnlockData(id) => assert_eq!(id.0, id1),
        other => panic!("expected UnlockData PDU, got {other:?}"),
    }
}
