...
fn neg_failure_on_protocol_mismatch() {
    ...
    let written = acceptor
        .step(&request_bytes, MonotonicInstant::ZERO, &mut output)
        .unwrap();
    assert!(matches!(written, Written::Nothing));

    // Step 2: acceptor tries to send confirm, finds no common protocol
    let mut output = WriteBuf::new();
    let result = acceptor.step(&[], MonotonicInstant::ZERO, &mut output);
...
fn neg_success_when_protocols_match() {
    ...
    let mut output = WriteBuf::new();
    let written = acceptor.step(&[], MonotonicInstant::ZERO, &mut output).unwrap();
    assert!(!matches!(written, Written::Nothing));
...
fn message_channel_advertised_when_client_requests_it() {
    ...
    acceptor
        .step(&request_bytes, MonotonicInstant::ZERO, &mut WriteBuf::new())
        .unwrap();
    acceptor
    ...
    acceptor
        .step(initial_buf.filled(), MonotonicInstant::ZERO, &mut WriteBuf::new())
        .unwrap();

    let mut output = WriteBuf::new();
    acceptor.step(&[], MonotonicInstant::ZERO, &mut output).unwrap();
