...
pub trait Session {
    ...
    /// Locks the remote clipboard for file transfer operations.
    ///
    /// This method sends a Lock Clipboard Data PDU to the remote and returns a `clipDataId`
    /// that must be used in subsequent file transfer operations. The lock prevents the
    /// clipboard from changing during the transfer.
    ///
    /// # Returns
