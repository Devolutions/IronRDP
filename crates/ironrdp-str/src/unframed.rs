...
impl UnframedString {
    ...
    pub fn decode(src: &mut ReadCursor<'_>, wchar_count: usize) -> DecodeResult<Self> {
        let byte_count = wchar_count
            .checked_mul(2)
            .ok_or_else(|| invalid_field_err!("wchar_count", "character count overflow", at: 0))?;
    ...
    pub fn decode_from_byte_len(src: &mut ReadCursor<'_>, byte_len: usize) -> DecodeResult<Self> {
        if byte_len % 2 != 0 {
            return Err(invalid_field_err!("byte_len", "odd byte count for utf-16 string field", at: 0));
        }
