...
pub const HYPERV_SECURITY_PROTOCOL: SecurityProtocol = SecurityProtocol::HYBRID.union(SecurityProtocol::SSL);
...
pub struct VmClientConnector {
    ...
    clinet_connector: ClientConnector, // hold it hostage, can't do anything with it until VMConnector handover
}
    ...
    fn step(&mut self, input: &[u8], output: &mut WriteBuf) -> ConnectorResult<Written> {
        let (written, next_state) = match mem::take(&mut self.state) {
            ...
            VmConnectorState::ConnectionInitiationWaitConfirm => {
                ...
                (Written::Nothing, VmConnectorState::Handover)
            }
...
impl VmClientConnector {
    ...
    pub fn hand_over(self) -> ClientConnector {
        assert!(
            self.should_hand_over(),
            "Cannot hand over VM connector in state: {}",
            self.state.name()
        );
        ...
        clinet_connector.state = ClientConnectorState::BasicSettingsExchangeSendInitial {
            selected_protocol: HYPERV_SECURITY_PROTOCOL,
        };
...
impl ironrdp_connector::SecurityConnector for VmClientConnector {
    ...
    fn credssp_selected_protocol(&self) -> Option<SecurityProtocol> {
