...
`ironrdp-client` registers `FrameBufferClient` for local Windows VMConnect sessions.
...
`ironrdp-client` supplies it automatically for VMConnect connections unless the caller set an explicit digital product ID.
