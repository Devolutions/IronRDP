...
impl iron_remote_desktop::SessionBuilder for SessionBuilder {
    ...
    /// for download. The clipboard is automatically locked when files are detected;
    /// the clipDataId is provided as a second parameter to the callback.
    ///
    /// # Callback signature:
    /// ```typescript
    /// function callback(files: Array<{name: string, path?: string, size: number, lastModified: number, isDirectory: boolean}>, clipDataId?: number): void
    /// ```
    ///
    /// ## Parameters
    /// - `files`: Array of file metadata objects, each containing:
    ///   - `name`: File basename (including extension, without directory path). Max 259 characters per MS-RDPECLIP spec.
    ///   - `path`: Optional relative directory path within the copied collection (e.g., `"temp\\subdir"`). Uses `\` as separator per the wire protocol. Absent for root-level files.
    ///   - `size`: File size in bytes. 0 indicates unknown size or empty file.
    ///   - `lastModified`: Last write time as milliseconds since the Unix epoch (matching JavaScript's `Date.getTime()`). 0 indicates unknown.
    ///   - `isDirectory`: Whether this entry represents a directory rather than a file.
    /// - `clipDataId`: The lock ID for downloading files. Undefined if
    ///   CAN_LOCK_CLIPDATA was not negotiated.
    ///
    /// ## Protocol Flow
    /// This callback is triggered when the remote user copies files. The file list follows
    /// [MS-RDPECLIP] 2.2.5.2 (FormatDataResponse with FileGroupDescriptorW format).
    /// To download files, the client must:
    /// 1. Use the `clipDataId` from this callback
    /// 2. Send FileContentsRequest PDUs for each file (via `requestFileContents()`)
    ///
    /// Lock lifecycle (expiry, cleanup, Unlock PDUs) is managed automatically.
    fn files_available_callback(&self, callback: js_sys::Function) -> Self {
        self.0.borrow_mut().files_available_callback = Some(callback);
        self.clone()
    }
    ...
    /// read the requested file chunk and respond via submit_file_contents().
    ///
    /// # Callback signature:
    /// ```typescript
    ...
    ///
    /// ## Parameters
    /// - `streamId`: Unique identifier for this file transfer stream
    /// - `index`: File index in the file list (0-based)
    /// - `flags`: FileContentsFlags bitmask:
    ///   - `0x1` (SIZE): Request file size (return 8-byte u64)
    ///   - `0x2` (RANGE): Request byte range
    /// - `position`: Byte offset for RANGE requests
    /// - `size`: Number of bytes requested for RANGE requests
    /// - `dataId`: Optional clipboard lock ID from LockClipData PDU
    ///
    /// ## Protocol Flow
    /// This callback is triggered when the remote pastes files that the client previously copied.
    /// Follows [MS-RDPECLIP] 2.2.5.3.1 (FileContentsRequest PDU).
    /// The client should read the requested file chunk and respond via `submitFileContents()`.
    fn file_contents_request_callback(&self, callback: js_sys::Function) -> Self {
        self.0.borrow_mut().file_contents_request_callback = Some(callback);
        self.clone()
    }
    ...
    /// # Callback signature:
    /// ```typescript
    /// function callback(response: {streamId: number, isError: boolean, data: Uint8Array}): void
    /// ```
    ///
    /// ## Parameters
    /// - `streamId`: Unique identifier for this file transfer stream
    /// - `isError`: If true, the request failed (data unavailable/access denied)
    /// - `data`: Response data:
    ///   - For SIZE requests: 8-byte little-endian u64
    ///   - For RANGE requests: requested byte range
    ///
    /// ## Protocol Flow
    /// This callback is triggered in response to client's FileContentsRequest.
    /// Follows [MS-RDPECLIP] 2.2.5.3.2 (FileContentsResponse PDU).
    fn file_contents_response_callback(&self, callback: js_sys::Function) -> Self {
        self.0.borrow_mut().file_contents_response_callback = Some(callback);
        self.clone()
    }
    ...
    /// # Callback signature:
    /// ```typescript
    /// function callback(dataId: number): void
    /// ```
    ///
    /// ## Parameters
    /// - `dataId`: Clipboard data ID that identifies the locked clipboard snapshot
    ///
    /// ## Protocol Flow
    /// This callback is triggered when the remote sends a LockClipData PDU.
    /// Follows [MS-RDPECLIP] 2.2.4.1. The clipboard remains locked until
    /// UnlockClipData PDU received.
    fn lock_callback(&self, callback: js_sys::Function) -> Self {
        self.0.borrow_mut().lock_callback = Some(callback);
        self.clone()
    }
    ...
    ///
    /// ## Parameters
    /// - `dataId`: Clipboard data ID from the corresponding lock operation
    ///
    /// ## Protocol Flow
    /// This callback is triggered when the remote sends an UnlockClipData PDU.
    /// Follows [MS-RDPECLIP] 2.2.4.2.
    fn unlock_callback(&self, callback: js_sys::Function) -> Self {
        self.0.borrow_mut().unlock_callback = Some(callback);
        self.clone()
    }

    /// Sets a callback that is invoked when client-side clipboard locks expire.
    ///
    /// This callback is triggered when automatic cleanup removes locks that have
    /// been inactive for too long or exceeded maximum lifetime. The locks have
    /// already been unlocked by the time this notification is sent.
    ///
    /// ## Parameters
    /// - `clipDataIds: Uint32Array` - Array of expired lock IDs
    ///
    /// ## Usage
    ...
    /// ## Example
    /// ```js
    /// .locksExpiredCallback((clipDataIds) => {
    ///     console.log(`Locks expired: ${clipDataIds}`);
    ///     fileTransferManager.handleLocksExpired(clipDataIds);
    /// })
    /// ```
    fn locks_expired_callback(&self, callback: js_sys::Function) -> Self {
        self.0.borrow_mut().locks_expired_callback = Some(callback);
        self.clone()
    }
...
impl iron_remote_desktop::Session for Session {
    ...
    fn submit_file_contents(&self, stream_id: u32, is_error: bool, data: Vec<u8>) -> Result<(), Self::Error> {
        self.input_events_tx
            .unbounded_send(RdpInputEvent::ClipboardBackend(
    ...
    fn initiate_file_copy(&self, files: JsValue) -> Result<(), Self::Error> {
        // Convert JsValue array to Vec<FileMetadata>
        ...
        for i in 0..js_array.length() {
            ...
            // Reject fractional or out-of-range values to avoid silent truncation.
