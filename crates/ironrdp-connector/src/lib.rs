...
use ironrdp_pdu::nego::{self, NegoRequestData, SecurityProtocol};
...
pub trait ConnectorCore: Sequence + SecurityConnector + CredsspSequenceFactory + Any {
    fn as_sequence(&mut self) -> &mut dyn Sequence;
    fn as_security_connector(&mut self) -> &mut dyn SecurityConnector;
