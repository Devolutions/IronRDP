...
use crate::credssp::{self, CredsspSequence};
...
impl SecurityConnector for ClientConnector {
    ...
    fn credssp_selected_protocol(&self) -> Option<nego::SecurityProtocol> {
        match &self.state {
...
impl CredsspSequenceFactory for ClientConnector {
    fn init_credssp(
        ...
        let (sequnece, ts_request) = credssp::CredsspSequence::init(
            credentials,
            domain,
            protocol,
            server_name,
        ...
        Ok((Box::new(sequnece), ts_request))
    }
}
