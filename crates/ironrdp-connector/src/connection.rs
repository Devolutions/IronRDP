...
mod tests {
    ...
    fn dyn_vc_gfx_config(support_dyn_vc_gfx_protocol: bool) -> Config {
        Config {
            desktop_size: DesktopSize {
                width: 1024,
                height: 768,
    ...
    fn early_capability_flags(config: &Config) -> gcc::ClientEarlyCapabilityFlags {
        create_gcc_blocks(config, nego::SecurityProtocol::SSL, true, core::iter::empty())
            .expect("GCC blocks")
            .core
            .optional_data
            .early_capability_flags
    ...
    fn dyn_vc_gfx_protocol_bit_is_absent_by_default() {
        assert!(
            !early_capability_flags(&dyn_vc_gfx_config(false))
                .contains(gcc::ClientEarlyCapabilityFlags::SUPPORT_DYN_VC_GFX_PROTOCOL)
        );
    }
    ...
    fn dyn_vc_gfx_protocol_bit_is_set_when_enabled() {
        assert!(
            early_capability_flags(&dyn_vc_gfx_config(true))
                .contains(gcc::ClientEarlyCapabilityFlags::SUPPORT_DYN_VC_GFX_PROTOCOL)
        );
    }
    ...
    fn dyn_vc_gfx_protocol_opt_in_changes_exactly_one_bit() {
        let without = early_capability_flags(&dyn_vc_gfx_config(false));
        let with = early_capability_flags(&dyn_vc_gfx_config(true));
