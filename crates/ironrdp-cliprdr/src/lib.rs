...
pub(crate) struct FileTransferState {
    ...
    /// Per MS-RDPECLIP 2.2.5.3, this is a signed i32, but validated to be non-negative.
    pub file_index: i32,
...
pub struct Cliprdr<R: Role> {
    ...
    /// Last time automatic cleanup check was performed (milliseconds from backend clock).
    /// Used to throttle cleanup checks to avoid excessive overhead on high-frequency API calls.
    last_cleanup_check: Option<u64>,

    /// Minimum interval between automatic cleanup checks.
    /// Defaults to 5 seconds. Set to None to disable automatic cleanup.
    cleanup_check_interval: Option<core::time::Duration>,
...
impl<R: Role> Cliprdr<R> {
    ...
    /// Creates new CLIPRDR processor with default lock timeout and automatic cleanup policy
    ///
    /// Defaults:
    /// - Inactivity timeout: 60 seconds
    /// - Max lifetime: 2 hours
    /// - Automatic cleanup check interval: 5 seconds
    pub fn new(backend: Box<dyn CliprdrBackend>) -> Self {
        Self::with_all_config(
            ...
            Some(core::time::Duration::from_secs(5)),  // auto_cleanup_interval
    ...
    ///
    /// ## Parameters
    /// - `inactivity_timeout`: Cleanup if no FileContentsRequest for this duration after expiry
    ...
    /// Creates new CLIPRDR processor with custom lock timeout and cleanup policy
    ///
    /// ## Parameters
    /// - `inactivity_timeout`: Cleanup if no FileContentsRequest for this duration after expiry
    /// - `max_lifetime`: Force cleanup after this duration regardless of activity
    /// - `auto_cleanup_interval`: Interval between automatic cleanup checks during API calls (None = disabled)
    pub fn with_cleanup_config(
        backend: Box<dyn CliprdrBackend>,
        inactivity_timeout: core::time::Duration,
        max_lifetime: core::time::Duration,
        auto_cleanup_interval: Option<core::time::Duration>,
    ) -> Self {
        Self::with_all_config(
            backend,
            inactivity_timeout,
    ...
    pub fn with_all_config(
        ...
        Self {
            ...
            cleanup_check_interval: auto_cleanup_interval,
            transfer_timeout,
    ...
    pub fn initiate_copy(&mut self, available_formats: &[ClipboardFormat]) -> PduResult<CliprdrSvcMessages<R>> {
        // Lazily cleanup expired locks during API activity
        let mut messages = self.maybe_cleanup_locks();
        ...
        if R::is_server() {
            pdus.push(ClipboardPdu::FormatList(
                self.build_format_list(&effective_formats).map_err(|e| encode_err!(e))?,
            ));
        } else {
            match self.state {
                CliprdrState::Ready => {
        ...
        messages.extend(pdus.into_iter().map(into_cliprdr_message));
        Ok(messages.into())
    }
    ...
    ///
    /// Fast paths:
    /// - Returns immediately if no locks, requests, or locked file lists exist
    /// - Returns immediately if automatic cleanup is disabled
    /// - Returns immediately if minimum check interval hasn't elapsed
    ///
    /// This method is called at the start of public APIs to provide automatic cleanup
    /// without requiring background threads or explicit periodic calls.
    #[cfg_attr(feature = "__test", visibility::make(pub))]
    fn maybe_cleanup_locks(&mut self) -> Vec<SvcMessage> {
        // INVARIANT: nothing to clean up when all tracked collections are empty
        if self.outgoing_locks.is_empty()
            && self.sent_file_contents_requests.is_empty()
            && self.locked_file_list_activity.is_empty()
        {
            return Vec::new();
        }

        // Check if automatic cleanup is disabled
        let Some(check_interval) = self.cleanup_check_interval else {
            return Vec::new();
        };

        // Throttle checks to avoid overhead on every API call
        let now = self.backend.now_ms();
        if let Some(last_check) = self.last_cleanup_check {
            let elapsed = now.saturating_sub(last_check);
            if elapsed < u64::try_from(check_interval.as_millis()).unwrap_or(u64::MAX) {
                return Vec::new();
            }
        }

        self.last_cleanup_check = Some(now);

        // Delegate to internal implementation
        match self.cleanup_expired_locks_impl() {
            Ok(messages) => messages.into(),
            Err(e) => {
                warn!(%e, "Error during automatic lock cleanup");
                Vec::new()
            }
        }
    }

    /// Checks for expired locks past their timeout and sends Unlock PDUs
    ///
    /// Uses two-tier timeout policy:
    /// 1. Inactivity: Cleanup if no FileContentsRequest for 60s after expiry
    /// 2. Max lifetime: Force cleanup after 2h since lock creation, regardless of activity
    ///
    /// Can be called manually for explicit cleanup, or relies on automatic cleanup
    /// if enabled via [`Self::new()`] or [`Self::with_cleanup_config()`].
    /// Returns PDUs to send for locks that have timed out.
    ///
    /// ## Example
    /// ```no_run
    ...
    /// // Manual cleanup call (optional if automatic cleanup is enabled)
    /// let messages = cliprdr.cleanup_expired_locks()?;
    ...
    /// # }
    /// ```
    pub fn cleanup_expired_locks(&mut self) -> PduResult<CliprdrSvcMessages<R>> {
        self.cleanup_expired_locks_impl()
    }

    /// Internal implementation of lock cleanup logic.
    ///
    /// Separated from public API to allow both manual calls and automatic cleanup
    /// to share the same implementation.
    fn cleanup_expired_locks_impl(&mut self) -> PduResult<CliprdrSvcMessages<R>> {
        let now = self.backend.now_ms();
    ...
    pub fn request_file_contents(&mut self, mut request: FileContentsRequest) -> PduResult<CliprdrSvcMessages<R>> {
        ...
        if let Some(ref file_list) = self.remote_file_list {
            // INVARIANT: request.index >= 0 (validated during decode)
            let file_index = usize::try_from(request.index).map_err(|_| {
            ...
            if request.flags.contains(FileContentsFlags::RANGE) {
                ...
                // file_index validated above via try_from.
                if let Some(file_desc) = file_list.files.get(file_index) {
        ...
        let validated_files: Vec<FileDescriptor> = files
            ...
            .filter(|file| {
                ...
                if file.name.is_empty() {
                    ...
                    false
                } else if wire_len > MAX_FILENAME_LEN {
                    warn!(
                        name = %file.name,
...
impl<R: Role> SvcProcessor for Cliprdr<R> {
    ...
    fn process(&mut self, payload: &[u8]) -> PduResult<Vec<SvcMessage>> {
        // Lazily cleanup expired locks during normal message processing
        let mut messages = self.maybe_cleanup_locks();
                    ...
                    match self.locked_file_lists.get(&clip_data_id) {
                        ...
                        None => {
                            warn!(
                                stream_id = request.stream_id,
                                clip_data_id,
                                "Received FileContentsRequest with clipDataId but no locked file list found"
                            );
                            // Send error response - clipDataId doesn't match any active lock
                            let error_response = FileContentsResponse::new_error(request.stream_id);
                            let pdu = ClipboardPdu::FileContentsResponse(error_response.into_owned());
                            return Ok(vec![into_cliprdr_message(pdu)]);
                        }
        ...
        .map(|mut handler_messages| {
            // Prepend automatic cleanup messages before handler responses
            messages.append(&mut handler_messages);
            messages
        })
    }
...
impl<R: Role> Cliprdr<R> {
    ...
    pub fn __test_cleanup_check_interval(&self) -> Option<core::time::Duration> {
        self.cleanup_check_interval
    }

    pub fn __test_last_cleanup_check(&self) -> Option<u64> {
        self.last_cleanup_check
    }
}
