...
Lock cleanup happens **automatically** during normal API activity (every 5 seconds by default). No manual cleanup is required in typical usage.

Automatic cleanup runs during:
- `initiate_copy()` / `initiate_paste()` / `initiate_file_copy()`
- `request_file_contents()`
- `process()` (normal message processing)

Locks are cleaned up based on:
- **Inactivity timeout** (60s default): No FileContentsRequest activity
- **Maximum lifetime** (2h default): Force cleanup regardless of activity

This prevents resource leaks from stalled file transfers without requiring background threads or explicit periodic calls.
...
To handle cleanup entirely manually, pass `None` for `auto_cleanup_interval`:

```rust
use std::time::Duration;

let cliprdr = Cliprdr::with_cleanup_config(
    Box::new(my_backend),
    Duration::from_secs(60),   // inactivity_timeout
    Duration::from_secs(3600), // max_lifetime
    None,                      // Disable automatic cleanup
);

// Then call cleanup_expired_locks() in your own event loop
```
