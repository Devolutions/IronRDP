...
impl GuiContext {
    ...
    pub fn run(self, input_event_sender: crossbeam::channel::Sender<RdpInputEvent>) -> ! {
        let Self {
            window,
            event_loop,
        ...
        event_loop.run(move |event, _, control_flow| {
            *control_flow = ControlFlow::Wait;
            ...
            if is_closed(&input_event_sender) {
                control_flow.set_exit();
            }
        })
    }
...
fn send_fast_path_events(
    input_event_sender: &crossbeam::channel::Sender<RdpInputEvent>,
...
/// Returns `true` if the `input_event_sender` is closed.
///
/// It does this by sending the `DevNull` event, which is ignored in the input_event_receiver loop elsewhere.
fn is_closed(input_event_sender: &crossbeam::channel::Sender<RdpInputEvent>) -> bool {
    input_event_sender.send(RdpInputEvent::DevNull).is_err()
}
