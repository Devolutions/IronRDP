...
use ironrdp::pdu::rdp::capability_sets::{InputFlags, MajorPlatformType, RailSupportLevel};
...
/// Minimal connector config, consumed only by [`ConnectionActivationFactory::new`] (touched on
...
/// `ActiveStageBuilder`. Pointer settings mirror the recording so the framebuffer evolves identically
