...
use ironrdp_tokio::{
    mark_pcb_sent_by_rdclean_path, perfrom_credssp, run_until_handover, send_pcb, single_sequence_step_read,
...
async fn connect_ws(
    ...
    let mut credssp_done = perfrom_credssp(
        upgraded,
        connector.as_mut(),
        &mut upgraded_framed,
        server_name,
    ...
    S: FramedRead + FramedWrite,
{
    ...
    let should_upgrade = ironrdp_tokio::skip_connect_begin(connector.as_security_connector());
...
pub async fn downcast_back_to_client_connector(
    ...
    let connector = if connector.is::<VmClientConnector>() {
        ...
        run_until_handover(credssp_finished, framed, *connector).await?
    } else if connector.is::<ClientConnector>() {
        let connector = connector
            .downcast::<ClientConnector>()
