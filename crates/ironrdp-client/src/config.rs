...
impl ConfigBuilder {
    ...
    ///
    /// This is the native VMConnect behavior for local Hyper-V connections and avoids handling a
    /// reusable host password.
    #[must_use]
    ...
    pub fn build(mut self) -> anyhow::Result<Config> {
        ...
        #[cfg(all(windows, feature = "vmconnect"))]
        if self.vmconnect_current_user && matches!(self.transport, TransportKind::RDCleanPath { .. }) {
