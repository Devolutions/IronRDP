...
`from_bits_truncate` is likely wrong. `from_bits_retain` will simply ignore unknown bits, but do
...
This is making `from_bits_truncate` behaves exactly like `from_bits_retain` because all bits are
then considered to be known and defined. In this case `from_bits` also never fails and provides no
real benefit over `from_bits_retain`.
