...
| Which RDPIDD critical branch fires under concurrent Sandbox desktops? | Static analysis now proves that a RDPIDD/IddCx PnP problem becomes `DisconnectSession(sessionId, 17)` and wire `0x11`. The strongest branches are failed `IddCxAdapterDisplayConfigUpdateForContainer2` and virtual render-device loss followed by failed GPU/WARP recovery. Static code alone cannot choose between those sibling branches | Capture the RDPIDD autologger/in-flight record containing `AdapterBugCheck` major/minor/status or the preceding `AdapterMonitorUpdate*`, `Render device removed`, and `FallbackToWARP*` records; separately capture the origin of the survivor's later logoff |
