...
| Which host-global component owns the current IDD/presentation constraint? | Microsoft ActiveX reproduces the early disconnect, long staggering and low display load do not help, and channel redirection is irrelevant. Each VM has a separate worker, but both use host GPU mirroring and the RDP stack names `0x11` `IndirectDisplayDriverFailure`. No enabled host event channel identifies the caller | Read-only elevated stack at `CRDPWDUMXStack::SetErrorInfo(0x11)` or the corresponding IDD failure, correlated across both workers; separately capture the origin of the survivor's later logoff |
