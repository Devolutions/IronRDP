...
| Retail testing admitted only one concurrent full Sandbox desktop | Observed; later desktop returned `ERROR_REMOTE_SESSION_LIMIT_EXCEEDED` |

## Open attribution questions

| Question | Why it remains open | Narrow evidence needed |
| --- | --- | --- |
| Which runtime host component is the final RIM peer for `ILSClientService`? | `LSCSHostPolicy.dll` is proven to call `ConnectToParentPartition` and create a remote RIM proxy. Generic RDV transports contain an LSCS offer selector but no policy sink; a successful direct desktop left host `TermService` and `vmicrdv` stopped | Elevated before/after protected-worker module and image-load comparison across a real desktop admission, correlated with VMBus/RDV telemetry |
