...
| Desktop transport | Carry RDP-related control/data traffic and render/input state | guest `termsrv.dll`, `rdpcorets.dll`, RDV VDEV, named-pipe/VMBus transport, Enhanced Mode VDEVs |
...
flowchart TB
    ...
    subgraph host["Hyper-V host"]
        ...
        SYNTH[vmuidevices.dll / rdp4vs.dll]
    end
...
| `vmwp.exe` | Per-VM worker process that hosts configured VDEVs | Does not statically contain the LSCS endpoint or `ILSClientService` markers; it remains the likely runtime host boundary |
...
| `vmuidevices.dll` | Hyper-V Synthetic RDP and RDP Encoder VDEV implementation | Separate Enhanced Mode stack; not the Sandbox LSCS bridge |
| `rdp4vs.dll` | RDP4VS encoder engine loaded by `vmuidevices.dll` | Separate Enhanced Mode engine; no LSCS/RDV licensing markers |
...
| Guest desktop listener | Windows Sandbox transport configuration | Same type-2 VMBus guest listener observed |
| Guest desktop admission | LSCS/RDV policy route | Same LSCS/RDV policy route |

Direct lifecycle management therefore does not establish a new licensing identity or bypass desktop
admission. It changes who asks Windows to create the VM, not who implements the VM or authorizes
the guest desktop.
