...
| Windows Enterprise multi-session under AVD should take the multi-session branch | High static inference, not tested | `IncreaseTotalSessionCount` explicitly bypasses the one-session check when WVD is enabled; Microsoft documents Enterprise multi-session as allowing concurrent interactive sessions and Windows Sandbox as publishable in AVD with nested virtualization |
| Windows Server is a supported alternative Sandbox host | Ruled out by product requirements | Microsoft's Windows Sandbox installation requirements specify Windows 10 or Windows 11 |
